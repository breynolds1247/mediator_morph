...
void RooLagrangianMorphOptimizer::setupMorphFunc(){
  if(this->morphFunc){
    ...
    this->morphFunc = new RooLagrangianMorphFunc("morphing","morphing","","xsection",this->vertices,this->temporaries_list);
  }
}
...
void RooLagrangianMorphOptimizer::setEvaluator(Evaluator* eval, double presetUncertainty){
  this->evaluator = eval;
  this->presetUncertainty = presetUncertainty;
}
...
void RooLagrangianMorphOptimizer::targetFcn(Int_t&npar, Double_t* /*gin*/, Double_t&f, Double_t*par, Int_t /*flag*/){
  ...
  try {
    ...
    for(size_t ipar=0; ipar<pars.size();ipar++){
      penalty += std::pow(pars_limit[ipar] - pars[ipar],2); // TODO: either implement factor as class member or multiply at the end penalty instead of adding
    }
    f *= (1+penalty/npar);
...
RooLagrangianMorphOptimizer::RooLagrangianMorphOptimizer(const char* input, const char* benchmarks, const std::vector<RooArgList>& vertices, const std::vector<std::string>& xsInputs, TClass* containerType, const ParamCardSet& startvalues) :
  fXSContainerType(containerType)
{
  ...
  RooLagrangianMorphOptimizer::splitpath(benchmarks,this->benchmarkfilename,this->benchmarkobservable);

  for(const auto& v:vertices){
    this->vertices.push_back(v);
  }
  RooArgList sampleinputs;
  for(auto const& sample: xsInputs) {
    RooStringVar* v = new RooStringVar(sample.c_str(),sample.c_str(),sample.c_str());
    this->xsInputs.push_back(sample);
    sampleinputs.add(*v);
  }

  cloneFileContents(this->inputfilename,false);
  cloneFileContents(this->benchmarkfilename,true);

  // setup the XS helper
  this->xsHelper = new RooLagrangianMorphFunc("xsHelper","xsHelper","",inputobservable.Data(),this->vertices,sampleinputs);
...
void RooLagrangianMorphOptimizer::setup(const ParamCardSet& startvalues){
  RooArgList* parameterlist = this->xsHelper->getParameterSet();
  ...
  for(size_t i=0; i<this->fnSamples; ++i){
    ...
    RooArgList* parset = this->xsHelper->getParameterSet();
