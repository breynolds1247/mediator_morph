...
def fit_cross_sections(masses, dictionaries):
    ...
    for (dictname,whichdict) in dictionaries.iteritems():
        ...
        for gq in whichdict:
            ...
            g = ROOT.TGraph()
            n = 0
            for (mass,d) in whichdict[gq].iteritems():
                dir(d)
                if not 'theory' in d:
                    continue
                print mass,d['theory']
            ...
            g.SetMarkerStyle(8)
            g.Draw("p")
            func = ROOT.TF1("mp","[0]*x^(-4+[1])*(1-(x/7))^[2]",0,2.01)
            g.Fit(func,"Wm+")
            if makePlots:
                ROOT.gPad.Print("xsec_"+gq+"_"+dictname+".pdf")
...
def fit_acceptances(masses, dictionaries):
    ...
    for (dictname,whichdict) in dictionaries.iteritems():
        ...
        for gq in whichdict:
            ...
            g = ROOT.TGraph()
            n = 0
            # fit gq=0.1 data for 0.15 -- HACK because of missing points that allow stable convergence. checked that 0.1 fit is a good approximation to 0.15 data.
            gqchoice = gq
            if '0.15' in gq:
                gqchoice = '0.10'
            for (mass,d) in whichdict[gqchoice].iteritems():
                ...
                if not 'acc' in d:
                    continue
                print mass,d['acc']
                g.SetPoint(n,float(mass),d['acc'])
                n=n+1
            g.SetMarkerSize(2)
            g.SetMarkerStyle(8)
            g.Draw("p")
            # func = ROOT.TF1("poly2","[0]+[1]*x+[2]*x*x",0,2.01)
            # g.Fit("poly2","Wm+")
            # sigmoid?
            #func = ROOT.TF1("mp","[0]/(1+exp(-(x-[1])*[2]))",0,1.5)
            func = ROOT.TF1("mp","[0]/(1+exp(-(x-[1])*[2]))*(1+exp(-(x-[3])*[4]))",0,2)
            minfitx = 0.39
            maxfitx = 2
            ...
            g.Fit(func,"Wm+","",minfitx,maxfitx)
            # spline?
            # spline = ROOT.TSpline3("grs",g);
            # gf = ROOT.TGraph()
            # n=0
            # for i in range(0,2000):
            #     m = i*0.001
            #     gf.SetPoint(n,m,spline.Eval(m))
            #     n=n+1
            # gf.SetMarkerColor(ROOT.kRed)
            # gf.Draw("p")
            if makePlots:
                ROOT.gPad.Print("acc_"+gq+"_"+dictname+".pdf")
            fits.append( { 'selection': dictname ,
                           'gq': gq ,
                           'function' : func } )
...
def morph(inputs,dictionaries,selection,coupling,systematic,variation,outputMasses,savePlots=False,unbiasedTest=True):
